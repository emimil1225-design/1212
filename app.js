...
(() => {
      ...
      function loadWorlds() {
        ...
        const worlds = Array.isArray(stored) && stored.length ? stored : sourceWorlds;
        return worlds.length ? worlds.map(normalizeWorld) : [normalizeWorld({ name: "새 세계관" })];
      }
      ...
      function loadScenes() {
        return sceneHelpers.loadScenes({
          readJson,
          storage,
          sourceTemplates,
          createId
      ...
      function selectedPromptSourceLabel() {
        ...
        return [
          `캐릭터: ${character?.name || "선택 없음"}`,
          `상황: ${scene?.title || "선택 없음"}`,
          `세계관: ${world?.name || "선택 없음"}`
        ].join(" · ");
      }
      ...
      function renderPrompt(forceRegenerate = false) {
        ...
        el.promptMeta.textContent = selectedPromptSourceLabel();
      }
